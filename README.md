...
Copy `.env.example` to `.env.local` if you want custom values. `AUTH_SECRET` is optional for local dev and Vercel — the app uses a demo fallback when it is unset. For a real deployment, set `AUTH_SECRET` to a random string (`openssl rand -base64 32` works).
...
https://tentwenty-task-seven.vercel.app/

Deploy to Vercel with zero config — auth works out of the box. Optionally add `AUTH_SECRET` in the Vercel project settings for a custom secret.
